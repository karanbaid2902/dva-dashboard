...
div style=display flex; align-items center; justify-content center; margin-bottom 20px;
    img src=httpsupload.wikimedia.orgwikipediacommonsaa9Amazon_logo.svg width=120 alt=Amazon Logo
div
...
# Header
st.markdown('h1 style=text-align center;📊 Amazon Sales & Analytics Dashboardh1', unsafe_allow_html=True)
st.divider()
...
def generate_ecommerce_data()
    try
        ...
        payment_methods = ['Amazon Wallet', 'CreditDebit Card', 'Amazon Pay', 'COD']
        ...
        for i in range(num_transactions)
            ...
            data.append({
                ...
                'Date' date.date(),
                'Time' f{hour02d}{minute02d}{second02d},
                'Order_ID' f'AMZ{i08d}',
                'Category' category,
                'Product' product,
                'Region' region,
        ...
        st.error(fError generating data {str(e)})
        return pd.DataFrame()
...
try
    ...
    with tab1
        ...
        with col5
            st.metric(💹 Margin %, f{(total_profittotal_revenue100).1f}%)
